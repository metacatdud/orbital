	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := orbitalCfg.Save(cfgPath); err != nil {
			if errors.Is(err, config.ErrConfigWrite) {
				...
				prompt.Info(prompt.NewLine("If you are not conformable to run with sudo, create the file manually and copy the following contents to it"))
				fmt.Println()
				fmt.Println()
		...
		caCert, caKey, err := certificate.GenerateCA(dataPath)
		if err != nil {
			return err
		}
